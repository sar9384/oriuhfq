...
namespace ConsoleApp2
{
    class Program
    {
        ...
        static void MakeMessage(DateTime d)
        {
            ...
            {
                WriteLine("Напишите ваш текст");
                string zxc = ReadLine();
                ids.Add(ids.Count + 1); dates.Add(d); notes.Add(zxc);
                WriteLine("Ваша заметка была успешно создана, и отмечена с датой: " + d); ReadKey();
                Main();
            }
            else WriteLine("На данную дату невозможно создать заметку. попробуйте выбрать другую дату"); EnteringDate(1);
        }
